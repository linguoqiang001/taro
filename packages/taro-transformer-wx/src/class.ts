...
class Transformer {
  ...
  private wxsSourceMap = new Map<string, string[]>();
  ...
  constructor(
    ...
    this.wxsSourceMap = wxsSourceMap;
    this.compile()
  }
  ...
  parseRender() {
    ...
    this.wxsSourceMap.forEach((value, key) => {
      if (/\.wxs$/.test(key)) {
        this.result.template += `${this.result.template}<wxs module="${value}" src="${key}" /> \n`
      }
    })
